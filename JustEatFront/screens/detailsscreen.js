  ...
  const renderItem = ({ item }) => {
    return (
      <Restdetail
        ...
        restimg={result.image}
        openModal={() => setIsModalOpen(!isModalOpen)}
        setItem={(item) => setCartItem(item)}
      />
    );
  };
  ...
  return isConnected ? (
    result.length === 0 ? (
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <Animated.View style={[styles.header]}>
          <View style={styles.header_container}>
            ...
            <Animated.Text
              style={
                ([styles.titlestyle],
                { fontSize: 18, fontWeight: "bold", opacity: opacity })
              }
            >
              {result.name}
            </Animated.Text>
            <View style={{ flexDirection: "row" }}>
              {state?.user?.favourites.findIndex((item) => item.id === id) !==
                -1 && state.user !== null ? (
                <TouchableOpacity
                  onPress={() => remove_from_favourites()}
        ...
        <Animated.FlatList
          data={[
            { title: "Recommended", dishes: SendRecommended() },
            { title: "Menu", dishes: result.dishes },
          ]}
