...
const userReducer = (state, action) => {
  switch (action.type) {
    ...
    /*
        ...
        */
    case "SAVE_ADDRESS":
      let new_address_array = state.user.address.map((a) => {
        return { ...a };
      });
    ...
    case "SET_NEW_ADDRESS":
      let array = state.user.address.map((a) => {
        return { ...a };
      });
      let i;
      for (i = 0; i < array.length; i++) {
        array[i].save_as_current = false;
        if (i === action.payload) array[i].save_as_current = true;
      }
      return {
        ...state,
        user: {
    ...
    case "ORDER_RECEIVED":
      let order_arr = state.user.orders.map((a) => {
        return { ...a };
      });
      let j;
...
const Save_address = (dispatch) => async (item) => {
  let prev_user = await AsyncStorage.getItem("user");
  let user = JSON.parse(prev_user);
  let new_address_array = user.address.map((a) => {
    return { ...a };
  });
  for (key in new_address_array) {
    new_address_array[key].save_as_current = false;
...
const Set_address = (dispatch) => async (item) => {
  ...
  let address_array = _user.address.map((a) => {
    return { ...a };
  });
  let i;
  for (i = 0; i < address_array.length; i++) {
    address_array[i].save_as_current = false;
    if (i === item) address_array[i].save_as_current = true;
  }
...
const Received_order = (dispatch) => async (item) => {
  ...
  let order_array = user.orders.map((a) => {
    return { ...a };
  });
  let i;
  for (i = 0; i < order_array.length; ++i) {
    if (order_array[i].OrderId === item) {
      ...
      order_array[i].deliveredAt = new Date();
    }
  }
  let new_user = {
    ...user,
